...
in
{
  home.activation.ensureGhosttyAppearance = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    ...
    if [[ -f "$CONFIG" ]]; then
      ...
      ensure_line "background-opacity" "0.97"
      ensure_line "background-blur" "25"
      ensure_line "macos-titlebar-style" "transparent"
    fi
  '';
