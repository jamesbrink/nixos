...

{
  config =
    mkIf (config.services.resticBackups.enable or config.programs.restic-backups.enable or false)
      {
        # Create a restic wrapper function that loads credentials only when needed
        environment.shellInit = ''
          # Restic wrapper function - loads AWS credentials only when running restic
          restic() {
            (
              # Use a subshell to prevent environment pollution
              local config_dir=""
              ...
              if [ -n "$config_dir" ] && [ -f "$config_dir/s3-env" ]; then
                # Source AWS credentials only for this restic invocation
                set -a
              ...
              # Execute the actual restic command
              command restic "$@"
            )
          }
        '';

        # Configure for zsh if enabled - only define the wrapper function
        programs.zsh.interactiveShellInit = mkIf (config.programs.zsh.enable or false) ''
          # Restic wrapper function for zsh - loads AWS credentials only when running restic
          restic() {
            (
              # Use a subshell to prevent environment pollution
              if [ -f "$HOME/.config/restic/s3-env" ]; then
