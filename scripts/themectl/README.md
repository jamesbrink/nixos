...
- **Runtime automation** – `themectl apply`/`cycle` now rewrite VSCode + Cursor settings, poke the AppleScript reloaders, refresh Neovim via `nvr`, rewrite `~/.tmux.conf.local`, drive `hyprctl reload`/`swww img ~/.config/omarchy/current/background` on Linux, and reload Ghostty in place (`cmux reload-config` when running under cmux, otherwise the Cmd+Shift+, reload keystroke via AppleScript) for instant visual parity. VSCode/Cursor `settings.json` is treated as JSONC (comments and trailing commas survive) and only `workbench.colorTheme` is rewritten.
