...
- **Runtime automation** – `themectl apply`/`cycle` now rewrite VSCode + Cursor settings, poke the AppleScript reloaders, refresh Neovim via `nvr`, rewrite `~/.tmux.conf.local`, drive `hyprctl reload`/`swww img ~/.config/omarchy/current/background` on Linux, flip the OS appearance to match the theme's `kind` (macOS: System Events `appearance preferences`; Linux: `gsettings` color-scheme), reload cmux in place (`cmux reload-config`, macOS only — re-reads `~/.config/ghostty/config` + `cmux.json`; its sidebar/tab chrome follows the macOS appearance), and reload standalone Ghostty via the Cmd+Shift+, keystroke for instant visual parity. VSCode/Cursor `settings.json` is treated as JSONC (comments and trailing commas survive) and only `workbench.colorTheme` is rewritten.
